...
from anystorage.contrib.aliyun import AliyunObject
...
@pytest.fixture(scope="module")
def bucket(storage: AliyunStorage) -> AliyunBucket:
...
def _upload_temp_file(bucket: AliyunBucket, object_key: str, content: str) -> str:
    """辅助函数：创建临时文件并上传到 OSS，返回本地临时文件路径。"""
...
class TestObjectRead:
    ...
    @pytest.mark.order(1)
    def test_object_name(self, bucket: AliyunBucket):
        """AliyunObject 的 name 属性应等于 object_key。"""
    ...
    @pytest.mark.order(2)
    def test_object_bucket_name(self, bucket: AliyunBucket):
        """AliyunObject 的 bucket_name 属性应返回所属 Bucket 名称。"""
    ...
    @pytest.mark.order(3)
    def test_object_exists_true(self, bucket: AliyunBucket):
    ...
    @pytest.mark.order(4)
    def test_object_exists_false(self, bucket: AliyunBucket):
        """exists 对不存在的对象应返回 False。"""
        # 手动构造一个不存在的 AliyunObject（绕过 get_object 的 None 检查）
        obj = AliyunObject(object_key="non-existent-key-99999",
                           bucket=bucket,
                           storage=bucket.storage)
        assert obj.exists() is False

    @pytest.mark.order(5)
    def test_object_size(self, bucket: AliyunBucket):
        """size 应返回正确的对象字节数。"""
    ...
    @pytest.mark.order(6)
    def test_object_size_empty(self, bucket: AliyunBucket):
        """空对象的 size 应为 0。"""
        object_key = "test-obj/size_empty.txt"
        try:
            _upload_temp_file(bucket, object_key, "")
            obj = bucket.get_object(object_key)
            assert obj is not None
            assert obj.size == 0
        finally:
...
class TestObjectUpdate:
    """AliyunObject 更新相关测试（通过 Bucket 的 fput/fget 间接验证）。"""

    @pytest.mark.order(7)
    def test_overwrite_object(self, bucket: AliyunBucket):
    ...
    @pytest.mark.order(8)
    def test_download_content_matches(self, bucket: AliyunBucket):
        """fget 下载的内容应与上传内容一致。"""
...
class TestObjectDelete:
    ...
    @pytest.mark.order(9)
    def test_delete_existing_object(self, bucket: AliyunBucket):
        ...
        try:
            ...
            # get_object 应返回 None
            assert bucket.get_object(object_key) is None
        finally:
            # 如果对象仍存在则清理
            if bucket.get_object(object_key) is not None:
                bucket.delete_object(object_key)

    @pytest.mark.order(10)
    def test_get_object_after_delete(self, bucket: AliyunBucket):
        """删除后 get_object 应返回 None。"""
        object_key = "test-obj/get_after_delete.txt"
        try:
            _upload_temp_file(bucket, object_key, "will delete")
            assert bucket.get_object(object_key) is not None
    ...
    def test_objects_list_after_delete(self, bucket: AliyunBucket):
