...
from anystorage.contrib.aliyun_storage import AliyunBucket
from anystorage.contrib.aliyun_storage import AliyunObject
...
class TestBucketCreate:
    ...
    @pytest.mark.order(2)
    def test_ensure_bucket_idempotent(self, storage: AliyunStorage):
...
class TestBucketRead:
    ...
    def test_get_bucket_not_found(self, storage: AliyunStorage):
        ...
        bucket = storage.get_bucket("test-existent-bucket-xyz-99999")
...
class TestBucketObjectOperations:
    ...
    @pytest.mark.order(8)
    def test_fput_and_fget(self, bucket: AliyunBucket):
        """fput 上传文件后，fget 应能下载到相同内容。"""
        object_key = "test-crud/fput_fget.txt"
        content = "hello any-storage bucket crud test"
    ...
    @pytest.mark.order(9)
    def test_objects_list(self, bucket: AliyunBucket):
        """objects 应返回已上传的对象列表。"""
    ...
    def test_get_object(self, bucket: AliyunBucket):
        """get_object 应返回正确的 AliyunObject 实例。"""
        ...
        try:
            ...
            assert isinstance(obj, AliyunObject)
    ...
    def test_get_object_not_found(self, bucket: AliyunBucket):
        """get_object 对不存在的对象应返回 None。"""
        obj = bucket.get_object("non-existent-object-key-99999")
        assert obj is None

    @pytest.mark.order(12)
    def test_object_exists_and_size(self, bucket: AliyunBucket):
    ...
    def test_put_and_get(self, bucket: AliyunBucket):
        """put 上传字节数据后，get 应能读取到相同内容。"""
        object_key = "test-crud/put_get.txt"
        content = "hello any-storage aliyun put/get test"
